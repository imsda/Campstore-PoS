...
test('cash sales use no camper ledger, reduce stock once, and update cash box totals', () => {
  ...
  db.prepare('INSERT INTO items(id,name,cost_cents,category,active,stock_qty,updated_at) VALUES(?,?,?,?,?,?,?)').run('drink','Drink',125,'Food',1,5,now());
  ...
  const result = createSale({ saleType:'cash', cart:[{id:'snack',qty:1},{id:'drink',qty:2}], cashReceivedCents:1000, clientRequestId:'cash-sale-1', user });
  assert.equal(result.total_cents, 950);
  assert.equal(result.change_given_cents, 50);
  ...
  assert.equal(tx.change_given_cents, 50);
  const storedItems = JSON.parse(tx.items_json);
  assert.equal(storedItems[0].line_total_cents, 700);
  assert.equal(storedItems[1].line_total_cents, 250);
  const ev = db.prepare('SELECT * FROM cash_events WHERE transaction_id=?').get(tx.id);
  assert.equal(ev.event_type, 'cash_sale');
  assert.equal(ev.sale_total_cents, 950);
  assert.equal(ev.net_drawer_change_cents, 950);
  ...
  assert.equal(cur.summary.cash_sales_cents, 950);
  assert.equal(cur.summary.cash_received_cents, 1000);
  assert.equal(cur.summary.change_given_cents, 50);
  assert.equal(cur.summary.expected_cash_cents, 10950);
  assert.ok(cur.activity.some(a => a.type === 'Cash Sale' && a.person_or_sale === 'Walk-up Cash Sale'));
  assert.equal(cur.walk_up_sales.length, 1);
  assert.equal(cur.walk_up_sales[0].purchase.details_available, true);
  assert.equal(cur.walk_up_sales[0].purchase.items.length, 2);
  assert.equal(cur.walk_up_sales[0].cash_received_cents, 1000);
  assert.equal(cur.walk_up_sales[0].change_given_cents, 50);
});
